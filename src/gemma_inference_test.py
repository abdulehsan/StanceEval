"""
gemma_inference_test.py — Zero-shot stance detection on the blind test set (ground_truth.csv)
using Gemma 4 31B via Cerebras with the best performing setup (V1 prompt, Arabic target name).

Evaluates on the 352 test rows, outputs raw predictions, and supports resumability.
...
RESULTS_PATH = os.path.join(PRED_DIR, "results_gemma4_31b_cerebras_test.csv")
RAW_LOG_PATH = os.path.join(LOG_DIR, "results_gemma4_31b_cerebras_test_raw.jsonl")
...
# ── System Prompt (V1 - Best Performing) ──────────────────────────────────────
SYSTEM_PROMPT = (
...
def main() -> None:
    ...
    # Step 5: Write predictions to TXT
