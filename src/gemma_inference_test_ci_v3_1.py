"""
gemma_inference_test_ci_v3_1.py — Zero-shot stance detection on the blind test set (ground_truth.csv)
using Gemma 4 31B via Cerebras with the CI v3.1 prompt and temperature 0.1.

Usage:
    python src/gemma_inference_test_ci_v3_1.py
"""
...
RESULTS_PATH = os.path.join(PRED_DIR, "results_gemma4_31b_cerebras_test_ci_v3_1.csv")
RAW_LOG_PATH = os.path.join(LOG_DIR, "results_gemma4_31b_cerebras_test_ci_v3_1_raw.jsonl")
...
- Distinguish reporting from endorsement. Mentioning an event or policy does not by itself express a stance, unless it is framed positively (e.g. promoting, celebrating, or inviting participation), in which case it leans Favor. This positive framing must be about the event or policy itself — a tweet that merely references the date or occasion in passing (e.g. for an unrelated personal, commercial, or incidental purpose) does not by itself indicate Favor.
