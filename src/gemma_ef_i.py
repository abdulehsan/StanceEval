"""
gemma_ef_i.py — Zero-shot stance detection on the blind test set (ground_truth.csv)
using Gemma 4 31B via Cerebras with the EF-I prompt (execution-Favor / implementation
clause added) and temperature 0.1.

Usage:
    python src/gemma_ef_i.py
"""
...
RESULTS_PATH  = os.path.join(PRED_DIR, "results_gemma4_31b_cerebras_test_ef_i.csv")
RAW_LOG_PATH  = os.path.join(LOG_DIR,  "results_gemma4_31b_cerebras_test_ef_i_raw.jsonl")

# ── Model / API constants ─────────────────────────────────────────────────────
MODEL_ID    = "gemma-4-31b"
TEMPERATURE = 0.1   # same as best_86_61 / ci_v3 — do not change without isolated test
TOP_P       = 0.95
...
- Criticism of how a policy is implemented (cost, access, delays, execution) does not itself indicate opposition — if the writer supports the underlying goal while criticizing execution, the stance is still Favor.
...
# ── Rate-limit pacing logic (Cerebras 5 RPM / 150 RPH ceiling) ───────────────
...
def main() -> None:
    ...
    print(f"   Prompt:      EF-I (implementation-criticism clause)")
    ...
    for position, (_, row) in enumerate(run_df.iterrows(), start=1):
        ...
        if len(request_timestamps) >= 150:
            ...
            if rl_headers:
                reset_str = (
                    rl_headers.get("x-ratelimit-reset-requests-hour") or
            ...
            print(f"   Entering cooldown: sleeping for {sleep_time:.1f}s (~{sleep_time/60:.1f} min).")
