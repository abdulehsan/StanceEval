"""
gemma_inference_train.py — Zero-shot stance detection using Gemma 4 31B via Cerebras,
run on train.csv (not dev.csv) for error-mining purposes.

Purpose: surface sentiment≠stance error rows on train.csv (never touching dev.csv)
so they can be used as few-shot examples later, without leaking dev signal.

Same v1 prompt, same 100-row stratified selection logic (33/33/34), same rate
pacing as the original dev-set script. Output files are kept fully separate
from the dev-set results/logs to avoid any collision.

Usage:
    python src/gemma_inference_train.py
...
# NOTE: pointed at train.csv, not dev.csv — this is the only source-data change
DEV_CSV_PATH = os.path.join(DATA_DIR, "train.csv")

# NOTE: separate output files — never overlaps with dev-set results/logs
RESULTS_PATH = os.path.join(PRED_DIR, "results_gemma4_31b_cerebras_train_errmine.csv")
RAW_LOG_PATH = os.path.join(LOG_DIR, "results_gemma4_31b_cerebras_train_errmine_raw.jsonl")
...
# ── Results CSV schema ────────────────────────────────────────────────────────
RESULTS_FIELDNAMES = [
    "row_index",      # original 0-based index in train.csv
    "target",
    "gold_label",
    "predicted_label",
...
# NOTE: fixed to V1 — original dev script had this pointed at V3, which would
# have silently invalidated the train/dev comparison
...
# ── Step 1: Stratified 100-row selection (same logic as dev script) ────────────
def select_100_rows(source_df: pd.DataFrame) -> tuple[pd.DataFrame, list[int], str]:
    first_100 = source_df.head(N_ROWS)
    ...
    if min_count >= balanced_threshold:
        ...
        for target, n in per_target_n.items():
            slice_ = source_df[source_df["target"] == target].head(n)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Zero-shot stance detection using Gemma 4 31B via Cerebras, on train.csv for error mining."
    )
...
def main() -> None:
    ...
    print(f"   Source:           train.csv (error-mining pass — NOT dev.csv)")
    print(f"   Prompt:           SYSTEM_PROMPT_V1")
    ...
    print(
        f"\n⚠️ NOTE: This is a train.csv error-mining run, not a dev-set evaluation."
        f"\n    Favg2/accuracy above are informational only — use this run to pull"
        f"\n    sentiment≠stance error rows for few-shot examples, not as a benchmark score."
