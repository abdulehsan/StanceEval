"""
gemma_inference_zeroshot_arabic_target_temp01.py — Zero-shot stance detection using Gemma 4 31B via Cerebras
with Arabic target names, the v1 system prompt, and temperature=0.1.

Runs on the same 100-row stratified subset as the English baseline.

Usage:
    python src/gemma_inference_zeroshot_arabic_target_temp01.py
"""
...
RESULTS_PATH = os.path.join(PRED_DIR, "results_gemma4_31b_cerebras_zeroshot_arabic_target_temp01.csv")
RAW_LOG_PATH = os.path.join(LOG_DIR, "results_gemma4_31b_cerebras_zeroshot_arabic_target_temp01_raw.jsonl")

# ── Model / API constants ─────────────────────────────────────────────────────
MODEL_ID = "gemma-4-31b"

# Configuration as requested: temperature=0.1, zero-shot
TEMPERATURE = 0.1
TOP_P = 0.95
...
# ── Prompt (Forced to V1 prompt: zero-shot) ───────────────────────────────────
SYSTEM_PROMPT_V1 = (
...
# ── Main ──────────────────────────────────────────────────────────────────────
def main() -> None:
    # Force 100-row stratified subset, ignore arguments
    try:
    ...
    print(f"   Prompt:           V1 System Prompt (English) - Zero Shot")
    ...
    # Write cleaned TXT file (no padding)
    txt_path = results_path.replace(".csv", ".txt")
    write_pred_txt(y_pred, txt_path)
    print(f"Saved predictions to {txt_path}")

    # Evaluate using subset logic
    gold_sub = dev_df.iloc[selected_indices]
    tmp_gold = "temp_gold_arabic.csv"
    gold_sub.to_csv(tmp_gold, index=False)
    
    try:
        scores = run_official_eval(tmp_gold, txt_path)
        print("\nOfficial subset evaluation scores:")
